...
# Global variables for text file paths
OUTPUT_DIR = '/Users/ericsheen/Desktop/DeepAI_Research/Spatial-Scene-Dataset/label/output111'
PROCESSED_VIDEOS_FILE = os.path.join(OUTPUT_DIR, 'video_paths_processed_part111.txt')
UNPROCESSED_VIDEOS_FILE = os.path.join(OUTPUT_DIR, 'unprocessed_videos_part_111.txt')
...
def process_videos(video_paths, prompt, batch_size=5):
    ...
    for i in range(0, len(video_paths), batch_size):
        ...
        try:
            ...
            with open(os.path.join(OUTPUT_DIR, f'gemini_video_captions_batch_{batch_counter}.json'), 'w') as f:
                json.dump(output_data, f, indent=2)

        except Exception as e:
            print(f"Error processing batch {batch_counter}: {str(e)}")
            # Log the unprocessed videos in a separate file
            with open(UNPROCESSED_VIDEOS_FILE, 'a') as f:
                for video_path in batch:
                    f.write(f"{video_path}\n")
            print(f"Logged unprocessed videos to '{UNPROCESSED_VIDEOS_FILE}'")
...
def get_video_paths_from_file(file_path):
    with open(file_path, 'r') as f:
        video_paths = [line.strip() for line in f.readlines()]
    return video_paths

video_file = "/Users/ericsheen/Desktop/DeepAI_Research/Spatial-Scene-Dataset/label/output11/unprocessed_videos_part_11.txt"
video_paths = get_video_paths_from_file(video_file)
